...
右下角按钮和面板标题都应显示当前脚本版本。更新后请确认显示 **v2.5.0**；仍显示旧版本时，不要继续比赛，应关闭旧页面、在 Tampermonkey 中保存新脚本后重新打开主页面。
...
- 终局一出现，脚本先把完整单局写入该站点浏览器的 `IndexedDB`，再把紧凑赛程、计分和逐手摘要写入 `localStorage`，随后默认自动下载一份 `betagomoku-...-game-...json`。单局文件含黑白方、逐手坐标/颜色/学号、每次 `/api/exec` 的完整输入与网页返回、网页终局文案、手数和时间。
- “导出完整 JSON”和阶段自动导出会从 `IndexedDB` 重新组装全部完整棋谱；若任一完整棋谱缺失，脚本会明确报错并停止导出，不会用摘要冒充完整记录。
- v2.5.0 第一次加载会自动迁移旧版已经完成的全部棋局，不改变分组、签表、排名或进度。迁移期间不会启动比赛；完成后面板会显示“旧记录检查/迁移完成”。
...
- 任一完整棋谱或赛程保存失败时，自动续局会立即关闭，面板显示红色错误；脚本不会再从旧进度无限重跑同一对局。
- 脚本在无法截获网页实际落子时会拒绝启动，以免生成缺棋谱的“完整记录”。

请勿直接手动点击网页原来的 Start 或修改下拉框来绕过赛事助手；那样网页会照常比赛，但不会写入赛事赛程和本地档案。

同一届赛事只能保留一个 BetaGomoku 页面和一份启用的赛事助手脚本。脚本会在浏览器本机建立三分钟续约的运行锁：另一标签页或重复安装的脚本尝试启动时会被拒绝，避免同一局反复运行。若浏览器异常关闭，等待三分钟锁自动失效后再继续。

### 诊断脚本

若现场仍出现异常，可把 [betagomoku-debug.user.js](betagomoku-debug.user.js) 作为第二份 Tampermonkey 脚本安装。它只观察 Start 点击来源、选手选择、页面终局消息、页面生命周期和赛事状态变化，不会点击 Start，也不会修改赛果。页面左下角会出现橙色 `Debug` 按钮；复现一次后点击“导出调试 JSON”，把文件交给维护者即可。正常比赛不必启用这份诊断脚本。
