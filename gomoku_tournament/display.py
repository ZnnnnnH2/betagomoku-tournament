...
</style></head><body><header><h1>BetaGomoku 赛事看板</h1><div class="meta" id="meta">正在读取本地赛事文件…</div></header><main class="shell"><section class="stage" id="stage"></section><aside class="side"><section class="overview" id="overview"></section><section class="groups" id="groups"></section><section class="bracket" id="bracket" hidden></section></aside></main>
<script>
...
async function refresh(){try{const res=await fetch('/api/state',{cache:'no-store'});if(!res.ok)throw Error();render(await res.json())}catch(_){document.querySelector('#meta').textContent='看板暂时无法读取本地赛事文件'}}refresh();setInterval(refresh,1000);
</script></body></html>"""
...
def dashboard_payload(run_dir: Path) -> dict[str, Any]:
    ...
    return {"updated_at": live.get("updated_at") if live else state["created_at"], "live": live, "current": current, "groups": groups, "knockout": knockout_view, "completed_games": completed, "phase": phase}
